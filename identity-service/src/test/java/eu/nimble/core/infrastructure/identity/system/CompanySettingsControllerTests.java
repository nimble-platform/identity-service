...
//        companyTradeDetails.getPaymentMeans().add(new PaymentMeans("instruction note"));
        ...
        this.mockMvc.perform(put("/company-settings/" + company.getHjid().toString())
                .contentType(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.AUTHORIZATION, "Bearer DUMMY_TOKEN")
                .content(gson.toJson(companySettings)))
                .andExpect(status().isAccepted());
        ...
        // THEN: getting settings should be updated
        this.mockMvc.perform(get("/company-settings/" + company.getHjid().toString()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
        ...
        this.mockMvc.perform(get("/qualifying/" + company.getHjid().toString())
                .header(HttpHeaders.AUTHORIZATION, "Bearer DUMMY_TOKEN"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$.businessClassificationScheme.description[0].languageID", is("en")))
                .andExpect(jsonPath("$.party.partyIdentification[0].id", is(company.getHjid().toString())));
    }
    ...
    public void testCertificateManagement() throws Exception {
        ...
        String responseAsString = this.mockMvc.perform(get("/company-settings/" + company.getHjid().toString()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
